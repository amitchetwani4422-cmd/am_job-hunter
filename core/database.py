...
def _normalize_duplicate_job(conn: sqlite3.Connection, job_dict: dict, now: str) -> None:
    ...
    existing = conn.execute(
        "SELECT * FROM jobs WHERE id = ? OR (url != '' AND url = ?) "
        "OR (external_job_id != '' AND external_job_id = ? AND lower(company) = lower(?)) LIMIT 1",
        (job_dict["id"], job_dict.get("url", ""),
         job_dict.get("external_job_id", ""), job_dict.get("company", "")),
    ).fetchone()
    if not existing:
        return
...
def insert_job(job_dict: dict) -> str:
    ...
    try:
        ...
        job_dict.setdefault("external_job_id", "")
        job_dict.setdefault("remote_status", "")
        conn.execute("""
            INSERT INTO jobs (id, title, company, location, description, url,
                            source, external_job_id, remote_status, posted_date, discovered_at, tech_stack,
