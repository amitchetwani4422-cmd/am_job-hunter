...
from core.collector import run_collection, run_company_crawl, run_target_company_collection
...
@app.post("/api/company-careers/{company_key}/collect")
async def api_collect_target_company(company_key: str):
    """Run one curated official company board and return detailed diagnostics."""
    result = await run_target_company_collection(company_key)
    if "error" in result:
